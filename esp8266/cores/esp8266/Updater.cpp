...
bool UpdaterClass::begin(size_t size, int command, size_t _end, size_t _start, int ledPin, uint8_t ledOn) {
  if(_size > 0){
#ifdef DEBUG_UPDATER
  ...
  */
  ...
  if (command == U_FLASH) {
   ...
   else updateEndAddress = _end;
//    uintptr_t updateEndAddress = FS_start - 0x40200000;
 
    if (_start == 0) updateStartAddress = (updateEndAddress > roundedSize)? (updateEndAddress - roundedSize) : 0;
    else updateStartAddress = _start;
    ...
    if((FS_start + roundedSize > FS_end) && _end == 0) {
//    if(FS_start + roundedSize > EEPROM_start) {
      _setError(UPDATE_ERROR_SIZE);
      return false;
    }
